...
const renderSpacing = function(spacing, chordLength) {
   ...
   for(index = 0 ; index < spacing - chordLength ; index++) {
...
const progressionGenerator = function (getSection, getKey, getProgression, getIsHalfBar,getSpace) {
   
   keyLibrary (getKey)

   let progressionNumOnly = removeSuffix(getProgression)
   console.log(progressionNumOnly)
  
   // Assign actual chords to progression
   for (let index=0; index !==  getProgression.length; index++) {
   ...
   if (getIsHalfBar === false) {
      ...
      for(let index = 0; index !== progressionNumOnly.length; index++) {
         ...
         if (chords[index].length === 1) {    
              resultString = resultString + `[${chords[index]}]` + renderSpacing(getSpace, 0)               
           }  else {
            resultString = resultString + `[${chords[index]}]` + renderSpacing(getSpace, chords[index].length - 1) 
         } 
      ...
      if (progressionNumOnly !== undefined && progressionNumOnly.length % 4 !== 0) {
      ...
      return resultString
         ...
         // 
            ...
            if (chords[index].length === 1) {    
                 ...
                 resultString = resultString + `[${chords[index]}]` + renderSpacing(halfSpace, 0)    
                     ...
                     resultString = resultString + `[${chords[index]}]` + renderSpacing(halfSpace, chords[index].length - 1) 
              }
         ...
         if (progressionNumOnly !== undefined && progressionNumOnly.length % 2 !== 0) {
            
            resultString = resultString + `${renderSpacing(halfSpace + 3 ,0)}|\n\n`
...
document.querySelector('#prog-form').addEventListener('submit', function (e) {
   ...
   result.textContent = progressionGenerator(e.target.elements.inputSection.value,
                                             e.target.elements.inputKey.value,
                                             e.target.elements.inputProgression.value,
                                             getIsHalfBar,
                                             e.target.elements.inputSpacing.value)

   document.getElementById('outputResult').value = document.getElementById('outputResult').value + result.textContent
   console.log(document.getElementById('outputResult').value)
